...
fetch(staticSite ? "api/radar.json" : "api/radar.json").then((response) => { if (!response.ok) throw new Error("信号数据暂时无法读取"); return response.json(); }).then((data) => { state.data = data; render(); }).catch((error) => { $("#signal-feed").innerHTML = `<div class="empty-state error-state">${escapeHtml(error.message)}</div>`; });
