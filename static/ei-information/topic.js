...
const standardsHref = staticSite ? "standards.html" : "/standards";
...
fetch(`api/topic/${encodeURIComponent(topicId)}.json`).then((response) => { if (!response.ok) throw new Error("主题不存在"); return response.json(); }).then((data) => {
  ...
  document.querySelector("#topic-blocks").innerHTML = topic.blocks.map((block) => {
    const items = block === "standard" ? standardProjects.slice(0, 6).map((project) => `<a class="standard-mini-card" href="${standardsHref}"><span>标准项目</span><strong>${escapeHtml(project.title)}</strong><small>${escapeHtml(project.status || "状态待核")}</small></a>`).join("") : signals.filter((signal) => signal.module === block).slice(0, 8).map(signalCard).join("");
