...
function render(data) {
  ...
  document.querySelector("#home-topic-card").innerHTML = topic ? `<p class="eyebrow accent">CURRENT RADAR</p><h2>${escapeHtml(topic.name)}</h2><p>${escapeHtml(topic.description)}</p><div class="sidebar-topic-stats"><strong>${signals.length}</strong><span>条统一信号</span></div><a class="text-link" href="/topics/${encodeURIComponent(topic.id)}">进入主题雷达 ↗</a>` : '<div class="sidebar-note">暂无主题数据。</div>';
